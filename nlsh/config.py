...
class Config:
    ...
    def _validate_config(self, config: dict[str, Any]) -> None:
        ...
        for i, backend in enumerate(config["backends"]):
            ...
            if "api_key" in backend and isinstance(backend["api_key"], str):  # noqa: SIM102
            ...
            if "timeout" in backend:
                try:
                    ...
                    raise ConfigValidationError(  # noqa: B904
                        f"Backend {i} timeout must be a number"
                    )
            ...
            if "max_image_size_mb" in backend:
                try:
                    ...
                    if max_size <= 0:
                        raise ConfigValidationError(
                            f"Backend {i} max_image_size_mb must be positive"
                        )
                except ValueError:
                    raise ConfigValidationError(  # noqa: B904
        ...
        if "stdin" in config:
            ...
            if "max_tokens" in config["stdin"]:
                try:
                    ...
                    raise ConfigValidationError("stdin.max_tokens must be an integer")  # noqa: B904
        ...
        if "nlgc" in config:
            ...
            if "include_full_files" in config["nlgc"]:  # noqa: SIM102
    ...
    def _load_config_file(self, config_file: str) -> None:
        ...
        try:
            ...
            raise ConfigValidationError(f"Invalid YAML in config file: {e}")  # noqa: B904
        except Exception as e:
            raise ConfigValidationError(f"Error loading config file: {e}")  # noqa: B904
    ...
    def _apply_env_overrides(self) -> None:
        ...
        if "NLSH_STDIN_DEFAULT_BACKEND" in os.environ:
            try:
                self.config.setdefault("stdin", {})["default_backend"] = int(
                    os.environ["NLSH_STDIN_DEFAULT_BACKEND"]
                )
            except ValueError:
                pass
        ...
        if "NLSH_STDIN_DEFAULT_BACKEND_VISION" in os.environ:
            try:
                self.config.setdefault("stdin", {})["default_backend_vision"] = int(
                    os.environ["NLSH_STDIN_DEFAULT_BACKEND_VISION"]
                )
            except ValueError:
                pass
        ...
        if "NLSH_STDIN_MAX_TOKENS" in os.environ:
            try:
                self.config.setdefault("stdin", {})["max_tokens"] = int(
                    os.environ["NLSH_STDIN_MAX_TOKENS"]
                )
            except ValueError:
                pass
        ...
        if "NLSH_NLGC_DEFAULT_BACKEND" in os.environ:
            try:
                self.config.setdefault("nlgc", {})["default_backend"] = int(
                    os.environ["NLSH_NLGC_DEFAULT_BACKEND"]
                )
            except ValueError:
                pass
