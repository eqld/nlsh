...
try:
    ...
    print(
        "Error: tiktoken library is required. Install with: pip install tiktoken", file=sys.stderr
    )
    sys.exit(1)

from nlsh.image_utils import detect_input_type, is_image_type
...
def parse_args(args: list[str]) -> argparse.Namespace:
    ...
    # Encoding selection
    parser.add_argument(
        "--encoding", default="cl100k_base", help="Tokenizer encoding to use (default: cl100k_base)"
    )

    # Verbose mode
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Show breakdown by input source"
    )
...
def count_text_tokens(text: str, encoding_name: str) -> int:
    ...
    try:
        ...
        raise ValueError(f"Error with encoding '{encoding_name}': {str(e)}")  # noqa: B904
...
def count_image_tokens(image_data: bytes) -> int:
    ...
    try:
        ...
        raise ValueError(f"Error processing image: {str(e)}")  # noqa: B904
    ...
    try:
        ...
        if is_image_type(mime_type):
            ...
            try:
                ...
                raise ValueError(f"Error processing text file: {str(e)}")  # noqa: B904

    except FileNotFoundError:
        raise FileNotFoundError(f"File not found: {file_path}")  # noqa: B904
    except Exception as e:
        raise Exception(f"Error processing file '{file_path}': {str(e)}")  # noqa: B904
...
def process_stdin(stdin_data: bytes, encoding_name: str) -> tuple[int, str]:
    ...
    if is_image_type(mime_type):
        ...
        try:
            ...
            raise ValueError(f"Error processing STDIN text: {str(e)}")  # noqa: B904
...
def main() -> int:
    ...
    try:
        ...
        if args.verbose and results:
            for source, tokens, _data_type in results:
